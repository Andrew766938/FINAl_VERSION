#!/usr/bin/env python
"""
Добавить примеры постов в БД

Запуск: python init_posts.py
"""
...
POSTS = [
    ...
    {"title": "🚀 DevOps: автоматизация и развертывание", "content": "Docker и Kubernetes - стандарты в DevOps. Научитесь контейнеризировать ваши приложения!"},
...
async def init_posts():
    """Инициализировать посты в БД"""
    ...
    async with async_session() as session:
        try:
            ...
            if not user:
                print("❌ Нет пользователей в БД. Запустите приложение сначала.")
            ...
            result = await session.execute(select(PostModel).limit(1))
            existing_post = result.scalars().first()
            
            if existing_post:
                print("⚠️  Посты уже существуют в БД")
                count = await session.execute(select(PostModel))
                posts_count = len(count.scalars().all())
                print(f"Найдено {posts_count} постов")
                await engine.dispose()
                return
            
            print(f"\n📝 Добавляю {len(POSTS)} постов...")
            
            # Add posts
            for i, post_data in enumerate(POSTS, 1):
                # Alternate between users if multiple users exist
                post = PostModel(
            ...
            print(f"\n✅ Успешно добавлено {len(POSTS)} постов!")
...
if __name__ == "__main__":
    print("="*60)
    print("🚀 Инициализация примеров постов")
    print("="*60)
